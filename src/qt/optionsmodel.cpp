...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeShellAmount"))
        settings.setValue("nAnonymizeShellAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeShellAmount = settings.value("nAnonymizeShellAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeShellAmount"))
        SoftSetArg("-anonymizeshellamount", settings.value("nAnonymizeShellAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeShellAmount:
            nAnonymizeShellAmount = value.toInt();
            settings.setValue("nAnonymizeShellAmount", nAnonymizeShellAmount);
            emit anonymizeShellAmountChanged(nAnonymizeShellAmount);
            break;
        default:
            break;
        }
